...
from preprocessing import read_load_trace_data,read_load_trace_data_gen,preprocessing,preprocessing_gen
...
class MAPDataset_gen(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        past_ip_b = [x[1] for x in batch]
        past_page_b = [x[2] for x in batch]
        data=rearrange(np.array(past_b), '(b c) h w-> b c h w',c=cf.channels,h=cf.image_size[0],w=cf.image_size[1]) #batch/channel/height/width
        past_tensor=torch.Tensor(data).to(device)
...
def data_generator_gen(file_path,TRAIN_NUM,TOTAL_NUM,SKIP_NUM):
    #TRAIN_NUM=10
    _, eval_data = read_load_trace_data_gen(file_path, TRAIN_NUM,TOTAL_NUM,SKIP_NUM)
